# Saturation Validation Plan: Simulation vs. Real vLLM

## Overview

Validate simulation results (BLIS/Vidur) against real vLLM performance using GuideLLM for load generation.
...
**Goal:** Automatically start vLLM server, run GuideLLM benchmark at simulated max QPS, and save results.
...
```bash
# Single command to run validation
...
**Process:**
1. Load config and max QPS from results file
2. Start vLLM server with extracted parameters
3. Run GuideLLM benchmark at target QPS
4. Stop server and save results

**Output:** Specified directory with vLLM logs and GuideLLM benchmark results.

---

## Implementation Plan

Single file: `saturation_orchestrator.py`
...
### 1. Implement `saturation_orchestrator.py`
- [ ] Check and install GuideLLM if not present
- [ ] VLLMServerManager class with context manager
- [ ] Build vLLM command from config (map batch_size → max-num-seqs, etc.)
- [ ] Health check polling with timeout
- [ ] GuideLLM runner with flat data JSON structure
- [ ] Main CLI with argparse

### 2. Testing
- [ ] Test with `lowprefix_results.json` on H100
- [ ] Verify vLLM server starts and stops correctly
- [ ] Verify GuideLLM benchmark runs at target QPS
- [ ] Test with both BLIS and Vidur max QPS values

---

## Config to vLLM Parameter Mapping
...
- ✅ Config and max QPS loaded from results file
- ✅ vLLM server starts and responds to health checks
- ✅ GuideLLM benchmark completes at target QPS
